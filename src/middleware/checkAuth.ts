...
export const isConnected = (req: Request, res: Response, next: NextFunction) => {
	const connected = _.isEmpty((req as any).auth.user);
	if (connected) {
		next();
	} else {
...
export const isAdmin = (req: Request, res: Response, next: NextFunction) => {
	const result = (req as any).auth.user?.role === Role.admin;
	if (result) {
		next();
	} else {
...
export const isArtist = (req: Request, res: Response, next: NextFunction) => {
	const result = (req as any).auth.user?.role === Role.artist;
	if (result) next();
	else {
...
export const isManager = (req: Request, res: Response, next: NextFunction) => {
	const result = (req as any).auth.user?.role === Role.manager;
	if (result) next();
	else {
